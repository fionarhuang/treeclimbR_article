...
for (i in seq_along(avDat)) {
    ...
    loc.minP_0.01 <- lapply(outMin, FUN = function(x) {
        xx <- x$node[x$keep_0.01]
        rm_ancestor(node = xx, tree = rowTree(tse))
    })
    loc.minP_0.05 <- lapply(outMin, FUN = function(x) {
        xx <- x$node[x$keep_0.05]
        rm_ancestor(node = xx, tree = rowTree(tse))
    })
    loc.minP_0.1 <- lapply(outMin, FUN = function(x) {
        xx <- x$node[x$keep_0.1]
        rm_ancestor(node = xx, tree = rowTree(tse))
    })
    ...
    # miLineage part I
    loc1_0.01.MLA <- lapply(loc1_0.01.MLA, FUN = function(x) {
        rm_ancestor(node = x, tree = rowTree(tse))})
    loc1_0.05.MLA <- lapply(loc1_0.05.MLA, FUN = function(x) {
        rm_ancestor(node = x, tree = rowTree(tse))})
    loc1_0.1.MLA <- lapply(loc1_0.1.MLA, FUN = function(x) {
        rm_ancestor(node = x, tree = rowTree(tse))})
    ...
    # miLineage part II
    loc2_0.01.MLA <- lapply(loc2_0.01.MLA, FUN = function(x) {
        rm_ancestor(node = x, tree = rowTree(tse))})
    loc2_0.05.MLA <- lapply(loc2_0.05.MLA, FUN = function(x) {
        rm_ancestor(node = x, tree = rowTree(tse))})
    loc2_0.1.MLA <- lapply(loc2_0.1.MLA, FUN = function(x) {
        rm_ancestor(node = x, tree = rowTree(tse))})
    ...
    rate.Lasso <- rateFun(loc.Lasso, loc.Lasso, loc.Lasso, 
                          nSim, method = "lasso")
    # StructFDR
    loc.StructFDR_0.05 <- lapply(loc.StructFDR_0.05, FUN = function(x) {
        rm_ancestor(node = x, tree = rowTree(tse))})
    rate.StructFDR <- rateFun(loc.StructFDR_0.01, loc.StructFDR_0.05,
                        loc.StructFDR_0.1, nSim, method = "StructFDRFDR")
    
    # BH
    loc.bh_0.05 <- lapply(loc.bh_0.05, FUN = function(x) {
        rm_ancestor(node = x, tree = rowTree(tse))})
    rate.bh <- rateFun(loc.bh_0.01, loc.bh_0.05, 
                       loc.bh_0.1, nSim, method = "BH")
    
    # HFDR
    loc.HFDR_0.01 <- lapply(loc.HFDR_0.01, FUN = function(x) {
        rm_ancestor(node = x, tree = rowTree(tse))})
    loc.HFDR_0.05 <- lapply(loc.HFDR_0.05, FUN = function(x) {
        rm_ancestor(node = x, tree = rowTree(tse))})
    loc.HFDR_0.1 <- lapply(loc.HFDR_0.1, FUN = function(x) {
        rm_ancestor(node = x, tree = rowTree(tse))})
