...
# -------------- Data analysis -----------------
fullDAT <- lapply(tse_list, FUN = function(x) {
    assays(x, withDimnames = FALSE)[[1]]})
isLeaf <- rowLinks(tse_list[[1]])$isLeaf
y <- as.numeric(colData(tse_list[[1]])$group)-1
...
loc.Lasso <- lapply(loc.Lasso, FUN = function(x) {
    xx <- as.numeric(x)
    rowLinks(tse_list[[1]])$nodeNum[xx]
})
