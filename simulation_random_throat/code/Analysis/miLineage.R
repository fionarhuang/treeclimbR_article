...
# count table
tipDAT <- lapply(tse_list, FUN = function(x) {x[rowLinks(x)$isLeaf, ]})
tipCount <- lapply(tipDAT, FUN = function(x) {
    xx <- assays(x)[[1]]
...
taxCount <- lapply(tipCount, 
                   FUN = function(x){
                       xt <- t(x)
                       cn <- transNode(tree = Tree, node = colnames(xt))
                       colnames(xt) <- transNode(tree = Tree, 
                                                 node = cn,
                                                 use.alias = TRUE)
...
# case
cv <- gsub(pattern = "C", "", colData(tse_list[[1]])$group)
cv <- factor(cv)
